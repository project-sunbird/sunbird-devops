    ...
    try {
        ansiColor('xterm') {
            ...
            if (uploadStatus.currentResult == "SUCCESS") {
                ...
                customBuildStatus = build job: "Build/$module/PlayerCustom", parameters: [string(name: 'private_branch', value: "$private_repo_branch"), string(name: 'branch_or_tag', value: "$branch_or_tag", string(name: 'diksha_tenant_tag', value: "master")]

                if (customBuildStatus.currentResult == "SUCCESS") {
                    println ANSI_BOLD + ANSI_GREEN + "Build/$module/PlayerCustom succeeded. Notifying via email and slack.." + ANSI_NORMAL
                ...
                cdnStatus = build job: "Deploy/$envDir/$module/PlayerCDN", parameters: [string(name: 'private_branch', value: "$private_repo_branch"), string(name: 'branch_or_tag', value: "$branch_or_tag"), string(name: 'cdn_enable', value: "true")]

                if (cdnStatus.currentResult == "SUCCESS") {
                    println ANSI_BOLD + ANSI_GREEN + "Deploy/$envDir/$module/PlayerCDN succeeded. Notifying via email and slack.." + ANSI_NORMAL
