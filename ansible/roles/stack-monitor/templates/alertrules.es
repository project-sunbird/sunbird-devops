...
ALERT ElasticsearchTooFewNodesRunning
  ...
  ANNOTATIONS {description="There are only {% raw %}{{$value}}{% endraw %} < 3 ElasticSearch nodes running", summary="ElasticSearch running on less than 3 nodes"}
...
ALERT ElasticsearchHeapTooHigh
  ...
  FOR 15m
  LABELS {severity="critical"}
  ANNOTATIONS {description="The heap usage is over 90% for 15m", summary="ElasticSearch node {% raw %}{{$labels.node}}{% endraw %} heap usage is high"}
