                    ...
                    dir("$WORKSPACE") {
                        ...
                        dir("$repo_name") {
                            ...
                            withCredentials([usernamePassword(credentialsId: env.githubPassword, passwordVariable: 'gitpass', usernameVariable: 'gituser')]) {
                                ...
                                tagRefBranch = sh(script: "git ls-remote --tags origin ${params.release_branch}* | grep -o ${params.release_branch}.* | sort -V | tail -n1", returnStdout: true).trim()

                                if (tagRefBranch == "" || tagRefBranch == "${params.release_branch}" ) {
