...
# Check if the obsrv-charts.csv file exists
if [ ! -f "obsrv-charts.csv" ]; then
  echo "Error: obsrv-charts.csv file not found"
  exit 1
fi
...
while IFS=',' read -r chart_name chart_repo; do
  ...
  if [ -z "$chart_repo" ]; then
    echo "Error: Repository URL not found for $chart_name in obsrv-charts.csv"
    exit 1
  fi
  ...
  # else
    ...
    if ! helm upgrade --install "$chart_name" "$chart_repo" -n "$namespace" -f global-values.yaml -f obsrv-sample-values.yml ; then
      echo -e "\e[91mError installing $chart_name\e[0m"
   #   exit 1
   fi
    echo -e "\e[92m$chart_name is installed successfully\e[0m"
  # fi
done < obsrv-charts.csv
