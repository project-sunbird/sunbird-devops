...
# Check if the charts.csv file exists
if [ ! -f "charts.csv" ]; then
  echo "Error: charts.csv file not found"
  exit 1
fi
...
while IFS=',' read -r chart_name chart_repo; do
  ...
  # else
    ...
    if ! helm upgrade --install "$chart_name" "$chart_repo" -n "$namespace" -f global-values.yaml; then
