...
setupJobs(){
  ...
  while IFS="" read -r line; do
   ...
   if [[ ${arr[0]} != "dev" ]]; then
      ...
      find $JENKINS_TMP/Deploy/jobs/${arr[0]} -type d -path "*Summary*" -prune -o -name config.xml -exec sed -i '/upstreamProjects.*/d' {} \;
   fi
   ...
   for key in "${!arr[@]}"; do
      ...
      find $JENKINS_TMP/Deploy/jobs/${arr[$key]} -type d -path "*Summary*" -prune -o -name config.xml -exec sed -i '/upstreamProjects.*/d' {} \;
