...
setupJobs(){
  ...
  while IFS="" read -r line; do
   ...
   diffs=$(colordiff --color -r --suppress-common-lines --no-dereference -x 'nextBuildNumber' -x 'builds' -x 'last*' /var/lib/jenkins/jobs $JENKINS_TMP | wc -l)
   if [[ $diffs -eq 0 ]]; then
      echo -e "\e[0;33m${bold}No changes detected. Exiting...${normal}"
      exit
   fi
   colordiff --color -r --suppress-common-lines --no-dereference -x 'nextBuildNumber' -x 'builds' -x 'last*' /var/lib/jenkins/jobs $JENKINS_TMP
